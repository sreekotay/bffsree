...
# Same portable scan, with explicit shifted-byte word construction.
word-pack: CFLAGS = -Wall -Wextra -O3 -DNDEBUG -DBF_FAST -DBF_WORD_SCAN=1 -DBF_WORD_PACK=1 -DBF_CELL_BITS=$(CELL_BITS) -DBF_CELL_SIGNED=$(CELL_SIGNED) -DBF_OP_BUF_BITS=$(OP_BUF_BITS)
word-pack:
	$(CC) $(CFLAGS) $(LDFLAGS) -o $(TARGET) $(SRCS)
...
.PHONY: all debug release ref prof fast word word-pack cell16 cell32 clean test metrics bench compare test-compare
